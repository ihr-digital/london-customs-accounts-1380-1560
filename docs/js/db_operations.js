...
db.version(102).stores({
    ...
    // A connective no longer cuts a qualifier phrase in half: 636 more provenance
    // qualifiers carry their place, in 115 more ladings, and phrases like "tele de
    // Henauth" read as one commodity. Cached cargos hold their own annotations; the
    // provenance store refills with the corpus.
    console.warn("DB v102 — clearing ladings and cargos to pick up qualifiers across in/de");
    try {
        await trans.table("ladings").clear();
        await trans.table("cargos").clear();
    } catch (error) {
        console.error("Error clearing data on v102 upgrade:", error);
    }
});
...
db.version(103).stores({
    ...
    // Two curator decisions: "de Hispania" on goods is Spain (50 cargos), "de Vere"
    // on cloth is Veere (6). Cached cargos hold their own annotations; the
    // provenance store refills with the corpus.
    console.warn("DB v103 — clearing ladings and cargos to pick up Spain and Veere provenance");
    try {
        await trans.table("ladings").clear();
        await trans.table("cargos").clear();
    } catch (error) {
        console.error("Error clearing data on v103 upgrade:", error);
    }
});
...
db.version(105).stores({
    ...
    // 14,459 statuses recovered, most of them indigena / alienigena: a run of
    // statuses after a name belongs to that person, and the parser used to stop at
    // the first. Cached cargos hold their own annotations.
    console.warn("DB v105 — clearing ladings and cargos to pick up denizen / stranger statuses");
    try {
        await trans.table("ladings").clear();
        await trans.table("cargos").clear();
    } catch (error) {
        console.error("Error clearing data on v105 upgrade:", error);
    }
});
